...
export default function DashboardPage() {
  return (
    <div className="space-y-8 max-w-6xl mx-auto pb-12 relative">
      ...
      <div className="absolute top-0 right-0 w-64 h-64 bg-yellow-300 rounded-[var(--radius-comic)] border-[var(--border-comic)] border-black shadow-[var(--shadow-comic-lg)] -rotate-12 translate-x-1/2 -translate-y-1/3 opacity-20 pointer-events-none blur-sm"></div>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xl font-bold bg-white inline-block px-3 py-1 border-[var(--border-comic)] border-black rounded-[var(--radius-comic)] mt-2 shadow-[var(--shadow-comic-sm)] rotate-1">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
        ...
        <StatCard title="Bounties Collected" value="$45,231" icon={<DollarSign className="h-24 w-24" />} trend="+18% this month" bg="bg-green-400" text="text-black" cardClass="rotate-2 hover:-rotate-1 transition-transform" />
        <StatCard title="Threat Level" value="Minimal" icon={<Activity className="h-24 w-24" />} trend="All systems normal" bg="bg-yellow-400" text="text-black" cardClass="-rotate-2 hover:rotate-1 transition-transform" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 relative z-10">
        <div className="lg:col-span-2 space-y-6">
          <Card className="h-[450px] flex flex-col bg-white">
            ...
            <CardFooter>
              ...
              <Button variant="ghost" className="w-full sm:w-auto font-black shadow-none border-[2px] border-transparent hover:border-black hover:bg-yellow-200">Share Report</Button>
            </CardFooter>
          </Card>
        </div>
        ...
        <div className="space-y-6">
          <Card className="bg-white overflow-hidden">
            ...
            <CardFooter className="bg-gray-100 p-4 border-t-[var(--border-comic)] border-black flex justify-center">
              <Button variant="link" className="text-black font-black uppercase tracking-wider text-xs">View Full Rogues Gallery &rarr;</Button>
            </CardFooter>
          </Card>
        </div>
      </div>
...
function StatCard({ title, value, icon, trend, bg, text, cardClass }: { title: string, value: string, icon: React.ReactNode, trend: string, bg: string, text: string, cardClass?: string }) {
  return (
    <Card className={`${bg} ${text} overflow-hidden relative shadow-[var(--shadow-comic-lg)] ${cardClass || ''}`}>
      ...
      <CardHeader className="border-b-[var(--border-comic)] border-black/20 pb-4 z-10 relative">
      ...
      <CardFooter className="bg-black/10 border-t-[var(--border-comic)] border-black/20 py-3 mt-0 z-10 relative">
...
function ThreatItem({ name, location, status }: { name: string, location: string, status: string }) {
  ...
  return (
    <div className={`p-4 flex items-center justify-between cursor-pointer hover:bg-gray-100 transition-colors bg-white`}>
      <div>
      ...
      <div className={`text-[10px] font-black uppercase px-2 py-1 border-[2px] border-black rounded-[var(--radius-comic)] shadow-[1px_1px_0_0_rgba(0,0,0,1)]
        ${isDanger ? 'bg-red-400 text-white' : isTracking ? 'bg-yellow-400 text-black' : 'bg-green-400 text-black'} 
      `}>
        {status}
      </div>
    </div>
